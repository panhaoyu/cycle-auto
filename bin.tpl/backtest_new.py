...
tkrs = list(filter(lambda x: len(x) > 0, map(lambda x: x.strip().split()[0] + '.' + x.strip().split()[1],
                                             open(os.path.join("/datas2/share/alpha_bar/DestData/Meta", 'universe.txt'),
                                                  'r').readlines())))
...
def reach_data(file_name, datatype='float32'):
    ...
    clse = np.memmap('/datas2/share/alpha_bar/DestData/MinBar/' + file_name, dtype=datatype, mode='r',
                     shape=(3160 * 240, 4507))
...
def plot_eval(data, v, ret='alpha_market', weight='', ifrank=True, nan=0, start_date=0, end_date=0, complex_ver=False,
    ...
    turnover = (df.groupby(['dateStr', 'minTime'])['turnover1'].sum() / df.groupby(['dateStr', 'minTime'])[
        'turnover2'].sum()).fillna(0)
    ...
    if complex_ver:
        ...
        tmp.plot(title='IC %s , IR %s' % (round(tmp.mean(), 3), round(tmp.mean() / tmp.std(), 3)), rot=-30,
                 label='ic')  # calculate avg ic and ir
        ax1.grid()
        ax1.legend()
        ic = round(tmp.mean(), 3)
    ...
    df['short'] = df[ret][df['factor_backtest'] < 0] * df['factor_backtest'][
        df['factor_backtest'] < 0]  # rank as weight
    ...
    output = pd.DataFrame(index=list(set(tmp_all['year'].tolist())),
                          columns=['from', 'to', 'return', 'pnl_per_day', 'win_rate', 'sharpe', 'drawdown'])
    ...
    output['win_rate'] = tmp_all[tmp_all['average'] > 0].groupby('year')['average'].count() / tmp_all.groupby('year')[
        'average'].count()
    ...
    output['sharpe'] = tmp_all.groupby('year')['average'].mean() / tmp_all.groupby('year')['average'].std() * np.sqrt(
        242)
    output['win/loss'] = -tmp_all[tmp_all['average'] > 0].groupby('year')['average'].mean() / \
                         tmp_all[tmp_all['average'] < 0].groupby('year')['average'].mean()
    ...
    output.loc['summary'] = [min(tmp_all['dateStr'].tolist()), max(tmp_all['dateStr'].tolist()), sum_ret,
                             sum_pnl_per_day, sum_winrate, sum_sharpe, sum_dd, sumdd_start, sumdd_end, sum_win_ret]
...
for i in range(0, len(var)):
    ...
    try:
        ...
        if diff_param:
            ...
            df_res[var[i]] = v_list
            ic, longret, returns, pnl = plot_eval(df_res, var[i], weight=param[i][4], ifrank=True,
                                                  complex_ver=param[i][5], nan=param[i][6], start_date=param[i][7],
                                                  end_date=param[i][8], trimP=param[i][9], trimN=param[i][10],
                                                  winsorP=param[i][11], winsorN=param[i][12])
            ...
            df_res[var[i]] = v_list
            ic, longret, returns, pnl = plot_eval(df_res, var[i], ret='return', weight='rank', ifrank=True,
                                                  complex_ver=False, nan=0, start_date=0, end_date=0, trimP=0, trimN=0,
                                                  winsorP=0, winsorN=0)
